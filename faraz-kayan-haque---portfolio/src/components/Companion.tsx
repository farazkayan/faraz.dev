          ...
          content: (
            <div className="flex flex-col gap-1 text-sm text-[#C4C4CA]">
              ...
              <div className="grid grid-cols-[80px_1fr] gap-x-4 gap-y-1">
                <span className="text-white">help</span><span>Show available commands</span>
                ...
                <span className="text-white">fortune</span><span>Get questionable wisdom</span>
                <span className="text-white">8ball</span><span>Ask the terminal a question</span>
                <span className="text-white">roll</span><span>Roll a dice</span>
                ...
                <span className="text-white">man</span><span>Read a manual</span>
              </div>
              <div className="mt-2 grid grid-cols-[80px_1fr] gap-x-4">
   ...
   > ^ < `}</pre>
      ...
      case 'fortune':
        const fortunes = [
          ...
          "It compiles. Ship it.",
          "It's not a bug, it's an undocumented easter egg.",
      ...
      case '8ball':
        if (args.length < 2) {
          newHistory.push({ type: 'output', content: "Ask me a question first." });
        } else {
          const answers = [
            "🔮 Probably.",
            "🔮 Ask me again after debugging.",
            "🔮 Absolutely.",
            "🔮 The terminal refuses to comment.",
            "🔮 Outlook not so good."
          ];
          newHistory.push({
            type: 'output',
      ...
      case 'roll':
        const max = args[1] && !isNaN(parseInt(args[1])) ? Math.min(parseInt(args[1]), 1000) : 6;
        const result = Math.floor(Math.random() * max) + 1;
        newHistory.push({ type: 'output', content: `🎲 You rolled: ${result}` });
        break;
...
( o.o )
 > ^ <`}</pre>
      ...
      case 'man':
        if (args.length < 2) {
          newHistory.push({ type: 'output', content: "Usage: man <command>" });
        } else {
          const manCmd = args[1].toLowerCase();
          if (manCmd === 'suki') {
            newHistory.push({ type: 'output', content: "SUKI(1)\n\nNAME\n    suki — orange cat\n\nDESCRIPTION\n    A highly qualified terminal supervisor.\n\nBUGS\n    Demands food." });
          } else if (manCmd === 'matrix') {
            newHistory.push({ type: 'output', content: "MATRIX(1)\n\nNAME\n    matrix — digital rain\n\nDESCRIPTION\n    Take the red pill.\n\nBUGS\n    May contain agents." });
          } else if (manCmd === 'sudo') {
            newHistory.push({ type: 'output', content: "SUDO(8)\n\nNAME\n    sudo — execute a command as another user\n\nDESCRIPTION\n    You have no power here." });
          } else if (manCmd === 'hack') {
            newHistory.push({ type: 'output', content: "HACK(1)\n\nNAME\n    hack — breach the mainframe\n\nDESCRIPTION\n    Hollywood hacking simulator." });
          } else {
            newHistory.push({ type: 'output', content: `No manual entry for ${manCmd}` });
          }
        }
        break;
      ...
      case 'sudo':
        if (trimmed === 'sudo make me a sandwich') {
          ...
          newHistory.push({ type: 'output', content: "Nice try." });
        } else {
          newHistory.push({ type: 'output', content: "user is not in the sudoers file. This incident will be reported." });
        }
        break;
